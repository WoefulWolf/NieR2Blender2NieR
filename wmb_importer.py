...
def construct_materials(texture_dir, material, material_index=-1):
    ...
    for texturesType in textures.keys():
        ...
        if os.path.exists(texture_file):
            ...
            elif textures_type.find('enviroment') > -1:
                specular_maps[textures_type] = textures.get(texturesType)
    ...
    for i, textureID in enumerate(albedo_maps.values()):
        ...
        if os.path.exists(texture_file):
            ...
            if i > 0:
                ...
                colornode.hide = True
    # Albedo Links
    organic = shader_name [0:3] in {"eye", "har", "skn", "siv20_wxmvx", "sis20_xxmfx", "har00_sbXtX"} or shader_name [0:11] in {"ois02_sbceX", "sis20_xxmfx", "siv00_sxmvx", "siv20_wxmvx", "har00_sbXtX"} or shader_name [0:5] in {"ois02", "ois01", "siv00_sxmvx", "siv20_wxmvx", "har00_sbXtX"}  
   ...
   # Enviroment Nodes
    enviroment_nodes = []
    enviroment_sepRGB_nodes = []
    colornode = None
    for i, textureID in enumerate(enviroment_maps.values()):
        texture_file = "%s/%s.dds" % (texture_dir, textureID)
        if os.path.exists(texture_file):
            enviroment_image = nodes.new(type='ShaderNodeTexImage')
            enviroment_nodes.append(specular_image)
            enviroment_image.location = 0,i*-60
            enviroment_image.image = bpy.data.images.load(texture_file)
            enviroment_image.hide = True
                
            if i > 0:
                enviroment_image.label = "enviromentMap0" + str(i-1)
            else:
                enviroment_image.label = "enviromentMap2"
               

   # enviroment Links
    organic = shader_name [0:3] in {"eye", "har", "skn", "siv20_wxmvx", "sis20_xxmfx", "har00_sbXtX"} or shader_name [0:11] in {"eye", "sis20_xxmfx", "siv00_sxmvx", "siv20_wxmvx", "har00_sbXtX"}
    if len(enviroment_nodes) == 1 or (organic and len(enviroment_nodes) >= 1):
        enviroment_principled = links.new(enviroment_nodes[0].outputs['Color'], Background.inputs['Color'])
        if shader_name[4] == "0" and not organic :
            enviroment_link = links.new(enviroment_nodes[0].outputs['Color'], Background.inputs['Color'])
